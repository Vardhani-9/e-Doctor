...
if (isset($_POST[submit])) {
    ...
    if (mysqli_num_rows($qsqlappointment) >= 1) {
        ...
        $sql = "INSERT INTO appointment(appointmenttype,patientid,appointmentdate,appointmenttime,app_reason,status,departmentid,roomid,doctorid) values('ONLINE','$lastinsid','$_POST[appointmentdate]','$_POST[appointmenttime]','$_POST[app_reason]','Pending','$_POST[department]','$_POST[department]','$_POST[doct]')";
        if ($qsql = mysqli_query($con, $sql)) {
                                ...
                                <div class="appointment">
                                    ...
                                    <form method="post" action="" name="frmpatapp" onSubmit="return validateform()" class="appointment-form">
                                        <ul class="row">
                                            ...
                                            <li class="col-sm-6">
                                                <label><input placeholder="Pincode" type="text" class="form-control" name="pincode" id="pincode" value="<?php echo $rspatient['pincode'];  ?>" <?php echo $readonly; ?>><i class="icon-pin"></i>
                                                </label>

                                            </li>
                                            <li class="col-sm-6">
                                                <label><input placeholder="City" type="text" class="form-control" readonly name="city" id="city" value="<?php echo $rspatient['city'] == ""  ?  "Vijayawada"  :  $rspatient['city'];  ?>" <?php echo $readonly; ?>><i class="icon-pin"></i>
                                                </label>
                                            ...
                                            ?>
                                                <li class="col-sm-6">
                                                    <label>
                                                        <input placeholder="User Name" type="text" class="form-control" name="loginid" id="loginid" value="<?php echo $rspatient[loginid];  ?>" <?php echo $readonly; ?>><i class="icon-login"></i>
                                                    </label>
                                                ...
                                                <label>
                                                    ...
                                                    <select name="department" class="selectpicker" id="department">
                                                        ...
                                                        while ($rsdept = mysqli_fetch_array($qsqldept)) {
                                                            echo "<option value='$rsdept[departmentid]' data-dname='$rsdept[departmentname]' dname='$rsdept[departmentname]'>$rsdept[departmentname]</option>";
                                                        }
                                                        ?>
                                                    </select>
                                                ...
                                                <label>
                                                    <select name="location" id="location" class="selectpicker">
                                                        ...
                                                        while ($rslocation = mysqli_fetch_array($qsqllocation)) {
                                                            echo "<option value='$rslocation[location_id]' data-lname='$rslocation[location_name]' lname='$rslocation[location_name]'>$rslocation[location_name] ( $rslocation[city] ) </option>";
                                                        }
                                                        ?>
                                                    </select>
                                                    <i class="ion-pin"></i>
                                            ...
                                            <li class="col-sm-6">
                                                <label>
                                                    <select name="sort_order" class="selectpicker" id="sort_order">
                                                        <option value="fee_asc" selected="selected">Doctors by fee - Low to High</option>
                                                        <option value="fee_desc">Doctors by fee - High to Low</option>
                                                        <option value="rating_asc">Doctors by rating - Low to High</option>
                                                        <option value="rating_desc">Doctors by rating - High to Low</option>
                                                    </select>
                                            ...
                                            <li class="col-sm-6">
                                                <label>
                                                    <select name="doct" class="selectpicker" id="doctor">
...
<script type="application/javascript">
    ...
    function validateform() {
        if (document.frmpatapp.patiente.value == "") {
            ...
            return false;
        } else if (document.frmpatapp.doctor.value == "") {
            alert("Doctor should not be empty..");
            document.frmpatapp.doctor.focus();
            return false;
        }  else if (document.frmpatapp.location.value == "") {
            alert("Location should not be empty..");
            document.frmpatapp.location.focus();
            return false;
        }  else if (document.frmpatapp.department.value == "") {
            alert("Department should not be empty..");
            document.frmpatapp.department.focus();
            return false;
        }else if (document.frmpatapp.pincode.value == "") {
            alert("Pincode should not be empty..");
            document.frmpatapp.pincode.focus();
            return false;
        }else {
            return true;
        }
    ...
    var locationName = '';
     $('select#department' ).change(function(){
        ...
        departmentName = $(' select#department option:selected').attr("data-dname");
      if(departmentId > 0 && locationId > 0){
        alert('Fetching '+departmentName+' doctors from location '+locationName);
        //setTimeout(loadDoctorInfo(), 5000);
        loadDoctorInfo();
      }
      /* $("select#sample_request_laboratoryCategory [selected = 'selected']").removeAttr("selected");
      $("#sample_request_laboratoryCategory > option").each(function() {
  ...
  $('select#location' ).change(function(){
    locationId = $(this).val();
    locationName = $('select#location option:selected').attr("data-lname");
    if(departmentId > 0 && locationId > 0){
        alert('Fetching '+departmentName+' doctors from location '+locationName);
        //setTimeout(loadDoctorInfo(), 5000);
        loadDoctorInfo();
    ...
    function loadDoctorInfo(){
        ...
        "sortOrder" :  $('select#sort_order option:selected').val()
    };

        $.ajax({
            type: 'POST',
            url: 'loaddoctorslist.php',
            contentType: 'application/json; charset=utf-8',
            data: JSON.stringify(postData),
            //dataType: 'json',
            success: function(data){
                //var doctorsList  = JSON.parse("[" + data + "]");
                var doctorsList = data;
                //console.log(data);
                var $select = $('#doctor');
                $select.find('option').remove();
                //console.log($select);
                //$select.append(data); // return empty
               /*  $.each(JSON.parse("[" + doctorsList + "]"),function(index, item)
                {
                   console.log(item);                    
                    //$select.append('<option value=' + item.doctorId + '>' + item.doctorName +'('+item.departmentName+') (Rating'+item.doctorRating+'*)'+ + '</option>'); // return empty
                }); */

                i = 0;
                //console.log(doctorsList.length);
                while (i < doctorsList.length) {
                    
                    item = doctorsList[i];
                    $select.append('<option value=' + item.doctorid + '>' + item.doctorname +'('+item.departmentname+')</option>'); // return empty
                    console.log(item);
                    i++;
                }
